...
\PaperTitle{{\name} 技术规范} % Article title

\Authors{李辰星\textsuperscript{$\dagger$}, 杨光\textsuperscript{$\dagger$} } % Authors
...
\Abstract{
比特币及其追随者们的成功已向世界展示了在互不信任的匿名用户间达成的去中心化共识所蕴含的价值。
以此共识为基础，我们可以构建由全用户一致认同的公共账本或共享状态机。
概念上该状态机可具备图灵完备性，并因此成为一台由所有用户共享的、存储内容无法被任何个人或组织所篡改的“世界计算机”。
然而，现有底层共识系统的的低吞吐率极大限制了共享状态机的处理能力和应用规模。


{\name} 以高吞吐的共识网络为基础，实现了每秒可处理数千件事务的、图灵完备的状态机。
为达到每秒处理数千件事务的高吞吐率，{\name} 采用独创的树图（Tree-Graph）结构确保用户对所有区块的先后顺序快速达成共识。这种\tg 结构充分调用所有分叉区块，对系统安全性和吞吐率同时产生贡献。
本文将介绍 {\name} 的协议设计及其具体实现中的参数选择和规范。
}

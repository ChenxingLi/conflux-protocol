...
\label{sec:blaming}
	...
	直观而言，此域代表着一张投票，对象是提交正确状态的最后一个祖先区块。
	%Intuitively, this field represents the vote to the latest ancestor block which commits to a correct state, 
	即，当前区块的挖掘者认可该祖先区块由 {\bf 延迟状态根哈希}, {\bf 延迟收据根哈希}, {\bf 延迟日志过滤器哈希}, 和 {\bf blame} 组成的提交状态。
	...
	需要强调的是，我们不是为了区块合法性而对这些域进行检查。但是，提交一个错误状态可能导致区块奖励的丢失。
	...
	此外，因为 $\block$ 认可 $\parentf^{\left(\head(\block)_{m}+1\right)}(\block)$ 提交的系统状态，
	%Furthermore, since $\block$ agrees with the state committed in $\parentf^{\left(\head(\block)_{m}+1\right)}(\block)$,
	被 $\parentf^{\left(\head(\block)_{m}+1\right)}(\block)$ 所 \emph{blame} 的区块将同样被 $\block$ （间接地） \emph{blame}。
	%the blocks blamed by  $\parentf^{\left(\head(\block)_{m}+1\right)}(\block)$ is also (indirectly) blamed by $\block$, 
	从而我们递归地确定在 $\chain(\block)$ 上的所有被最新区块 $\block$ \emph{blame} 的区块。
	...
	我们进行一个最大努力测试来决定其是否被 $\block$ \emph{blame}。
	%we do a best effort test to decide whether they are blamed by $\block$.
	更确切地说，每个区块 $\block'$ 都确定了一组被 $\block'$ 沿着 $\chain(\block')$ 所 \emph{blame} 的区块。
	%More precisely, every block $\block'$ must have determined a set of blocks blamed by $\block'$ along $\chain(\block')$,
	
	若两者没有在交集 $\chain(\block)\bigcap\chain(\block')$ 中被 blame 的区块集合上达到完全一致，则 $\block'$ 将被 $\block$ \emph{blame}。
	...
	因此，我们确定了每个区块 $\block'$ 是否被任意其他区块 \emph{blame}。
	%Thus we have determined for every block $\block'$ whether it is blamed by any other block $\block$.
	对被 \emph{blame} 的区块（明确来说，被枢轴链上最后一个区块所 \emph{blame} 的区块）的惩罚在第 \ref{sec:incentive} 节中有具体说明。
	...
	当被 \emph{blame} 时将以不同方式处理，同第 \ref{sec:block} 节开头所简单描述的一样。
	%would be handled differently when being blamed, as briefly described at the beginning of Section~\ref{sec:block}.
	即，若 $\head_m>0$ 且状态承诺域被 \emph{blame}，那么该承诺将是一个由正确承诺组成的向量的 Keccak 哈希。
	...
	例如，若对于一个区块 $\block$，$\head(\block)_m=2$ 成立，即 $\parentf(\block),\parentf^{(2)}(\block)$ 被 \emph{blame} 而 $\parentf^{(3)}(\block)$ 没有，那么区块 $\block$ 的 {\bf 延迟状态根哈希} 域 $\head(\block)_r$ 应为：

...
\paragraph{基本项} 若 $\account_{basic}\neq \account^0_{basic}$，基本项包含基本组成部分 $\account_{basic}$。对于一个非合约地址，Conflux 不存储只与合约相关的域，例如代码哈希 $\account_c$，管理账户 $\account_a$，和资助信息 $\account_p$。
%\paragraph{Basic entry.} The basic entry stores basic components $\account_{basic}$ if $\account_{basic}\neq \account^0_{basic}$. Specially, for a non-contract address, Conflux doesn't stores the fields only related to contract, like code hash $\account_c$, contract admin $\account_a$ and sponsor info $\account_p$. 
...
\paragraph{存储根项} 若 $\mathsf{Type}_{\account_{addr}}\in \{\typereserved,\typecontract\}$ 且 $\account_n\neq 0$，存储根项记录着账户 $\account$ 的存储布局项。
...
\paragraph{代码项} 若 $\account_{code}\neq  \account^0_{code}$，代码项记录着代码信息部分 $\account_{code}$。 
...
Conflux 维持一个 \textbf{read-through write-back} 缓存，并根据该缓存在纪元结束时对 Merkle 前缀树进行更新。
%Conflux maintains a \textbf{read-through write-back} cache and commits cached changes to MPT at the end of epoch execution. 
...
若一个状态项在一个纪元执行后被添加、更新或移除，其最新的内容将被添加到增量集。
%After an epoch execution, if a state entry is added, updated or removed, its newest content will be added to the delta set. 
增量集只比较一个纪元执行前后的状态项，并不了解纪元执行时的中间步骤。
%Notice that the delta set only compares the state entry before and after the epoch execution. It is not aware of the intermediate steps during epoch execution. 
%
删除状态项时，在增量集中的该项值将被设为 $\varepsilon$。
...
\begin{itemize} [nosep]
	...
	\item {\bf 行动(action):} 
	一个变量大小域，表示此交易接收方所触发的行动，即 \textbf{调用} 一个地址或 \textbf{创建（creation）}，正式记作 $\tx_a\in \B_{160}\cup\B_0$。
	...
	对一笔 \textbf{调用} 交易而言，行动 $\tx_a$ 表示接收方的 $160$ 比特地址信息,代表着普通账户或合约账户； 
	...
	或当交易是一笔 \textbf{创建} 交易，则接收方为新创建的合同，且将 $\tx_a$ 认作 $\B_0$ 中的唯一元素，即 $\tx_a = \emptystring$。
...
\smallskip
除以上两种类型交易的共有域之外，一笔交易可能包含以下不限长度的字节序列域之一，用以创建和调用合约：
...
\newversion{
	区块头 $\head$ 的合法性仅与区块全序的共识有关，并不依赖于区块头内与执行相关的域( {\bf 延迟状态根哈希}, {\bf 延迟收据根哈希}, {\bf 延迟日志过滤器哈希}, 和 {\bf blame})的正确性，即 $\head$ 中 ${\head_r}, {\head_e}, {\head_b}$ 和 ${\head_m}$ 的正确性。
...
\label{sec:blaming}
	...
	那么，对于当下区块 $\block$ 而言，在 $\block$ 和 $\parentf^{\left(\head(\block)_{m}+1\right)}(\block)$ 之间的所有区块，即 $\parentf(\block), \cdots, \parentf^{\left(\head(\block)_{m}\right)}(\block)$，
	都填写了错误状态。
	...
	%the blocks blamed by  $\parentf^{\left(\head(\block)_{m}+1\right)}(\block)$ is also (indirectly) blamed by $\block$, 
	从而我们递归地确定在 $\chain(\block)$ 上的所有被最新区块 $\block$ 投反对票的区块。
	%and recursively we can determine all the blocks on $\chain(\block)$ that are blamed by the newest block $\block$.
	...
	%More precisely, every block $\block'$ must have determined a set of blocks blamed by $\block'$ along $\chain(\block')$,
	...
	此外，状态承诺域，即 {\bf 延迟状态根哈希}, {\bf 延迟收据根哈希}, {\bf 延迟日志过滤器哈希},
	...
	%That is, if $\head_m>0$ and the those fields are blamed, then the commitment will be a Keccak hash of the vector consisting of the correct commitments.
	...
	%For example, if a block $\block$ has $\head(\block)_m=2$ such that $\parentf(\block),\parentf^{(2)}(\block)$ are blamed but $\parentf^{(3)}(\block)$ is not, then the {\bf deferredStateRoot} field $\head(\block)_r$ of block $\block$ would be 
	\begin{align}
